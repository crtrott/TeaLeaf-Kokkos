...
struct GenerateChunk
{
	...
	GenerateChunk(
			...
			KView density, KView u, KView cellX, KView cellY, KView vertexX, KView vertexY, 
			KView stateXMin, KView stateXMax, KView stateYMin, KView stateYMax, 
			KView stateEnergy, KView stateDensity, 
		...
		vertexX(vertexX), vertexY(vertexY), stateXMin(stateXMin), 
		stateXMax(stateXMax), stateYMin(stateYMin), stateYMax(stateYMax),
		stateEnergy(stateEnergy), stateDensity(stateDensity), 
	...
	void operator()(const int index) const 
    {
		...
		if(kk > 0 && kk < dims.x-1 && jj > 0 && jj < dims.y-1) 
		{
