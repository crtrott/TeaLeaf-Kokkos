...
SUBROUTINE tea_exchange(fields,depth)
  ...
  IMPLICIT NONE
    ...
    IF(chunks(chunk)%chunk_neighbours(chunk_left).NE.external_face) THEN
      ...
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, left_right_offset, depth, &
            CHUNK_LEFT, chunks(chunk)%left_snd_buffer,1)
      else
    ...
    IF(chunks(chunk)%chunk_neighbours(chunk_right).NE.external_face) THEN
      ...
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, left_right_offset, depth, &
            CHUNK_RIGHT, chunks(chunk)%right_snd_buffer,1)
      else
    ...
    IF(chunks(chunk)%chunk_neighbours(chunk_left).NE.external_face) THEN
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, left_right_offset, depth, &
            CHUNK_LEFT, chunks(chunk)%left_rcv_buffer,0)
      else
    ...
    IF(chunks(chunk)%chunk_neighbours(chunk_right).NE.external_face) THEN
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, left_right_offset, depth, &
            CHUNK_RIGHT, chunks(chunk)%right_rcv_buffer,0)
      else
    ...
    IF(chunks(chunk)%chunk_neighbours(chunk_bottom).NE.external_face) THEN
      ...
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, bottom_top_offset, depth, &
            CHUNK_BOTTOM, chunks(chunk)%bottom_snd_buffer,1)
      else
    ...
    IF(chunks(chunk)%chunk_neighbours(chunk_top).NE.external_face) THEN
      ...
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, bottom_top_offset, depth, &
            CHUNK_TOP, chunks(chunk)%top_snd_buffer,1)
      else
    ...
    IF( chunks(chunk)%chunk_neighbours(chunk_top).NE.external_face ) THEN
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, bottom_top_offset, depth, &
            CHUNK_TOP, chunks(chunk)%top_rcv_buffer,0)
      else
    ...
    IF(chunks(chunk)%chunk_neighbours(chunk_bottom).NE.external_face) THEN
      if (use_ext_kernels) then
        call ext_pack_message(chunk,fields, bottom_top_offset, depth, &
            CHUNK_BOTTOM, chunks(chunk)%bottom_rcv_buffer,0)
      else
