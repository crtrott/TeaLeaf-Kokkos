		...
		const double* dz)
{
	...
	int n = std::max(std::max(c->dims.x,c->dims.y),c->dims.z);

    InitialiseChunkVertices<DEVICE> vertexKernel(c->dims, c->vertexX, c->vertexY, 
			c->vertexZ, c->cellX, c->cellY, c->cellZ, *xMin,
			*yMin, *zMin, *dx, *dy, *dz);
	PROFILED_PARALLEL_FOR(n, vertexKernel, "Initialise Chunk Vertices");

	InitialiseChunkAreas<DEVICE> areaKernel(c->volume, c->xArea, c->yArea, 
			c->zArea, *xMin, *yMin, *zMin, *dx, *dy, *dz);
		...
		const int* pointParam)
{
	...
	View<double*,DEVICE> dStateZMin("stateZMin",nStates);
	View<double*,DEVICE> dStateZMax("stateZMax",nStates);
	...
	typename View<double*>::HostMirror hStateZMin = create_mirror_view(dStateZMin);
	KokkosHelper::InitMirror<double>(hStateZMin,stateZMin,nStates);

	typename View<double*>::HostMirror hStateZMax = create_mirror_view(dStateZMax);
	KokkosHelper::InitMirror<double>(hStateZMax,stateZMax,nStates);
	...
	deep_copy(dStateZMin,hStateZMin);
	deep_copy(dStateZMax,hStateZMax);
	...
	GenerateChunk<DEVICE> kernel(c->dims, nStates, *rectParam, *pointParam, *circParam, 
			c->energy0, c->density, c->u, c->cellX, c->cellY, c->cellZ, c->vertexX, c->vertexY,
			c->vertexZ, dStateXMin, dStateXMax, dStateYMin, dStateYMax, dStateZMin, dStateZMax,
...
#define PRE_KERNEL(len, buf)\
			...
			switch(*face)
			{
				...
				case CHUNK_BACK:
					{
						PRE_KERNEL(c->fbLineLen**depth, c->fbBuffer);

						FrontBackPacker<DEVICE> kernel(c->dims, dBuf, field, *depth, *face, *pack);
						PROFILED_PARALLEL_FOR(bLen, kernel, "Packing F/B");
						break;
					}
				default:
        ...
        const int* chunk)
{
    ...
    CalculateResidual<DEVICE> kernel(
            c->dims, c->u, c->u0, c->r, c->kx, c->ky, c->kz);
...
#define UPDATE_FACE(face,buffer,n) \
    ...
    {\
        ...
        PROFILED_PARALLEL_FOR(n, kernel, "Update Halo " + std::to_string(face));\
    }

#define LAUNCH_UPDATE(index, buffer)\
    if(fields[index-1])\
    {\
        UPDATE_FACE(CHUNK_LEFT, buffer, (dims.y-HP2)*(dims.z-HP2)**depth);\
        UPDATE_FACE(CHUNK_RIGHT, buffer, (dims.y-HP2)*(dims.z-HP2)**depth);\
        UPDATE_FACE(CHUNK_TOP, buffer, (dims.x-HP2)*(dims.z-HP2)**depth);\
        UPDATE_FACE(CHUNK_BOTTOM, buffer, (dims.x-HP2)*(dims.z-HP2)**depth);\
        UPDATE_FACE(CHUNK_FRONT, buffer, (dims.x-HP2)*(dims.y-HP2)**depth);\
        UPDATE_FACE(CHUNK_BACK, buffer, (dims.x-HP2)*(dims.y-HP2)**depth);\
    }
        ...
        const double* dt)
{
    ...
    double rz = *dt/(c->dz*c->dz);

    JacobiInit<DEVICE> kernel(c->dims, c->u, c->u0, c->density, c->energy,
            c->kx, c->ky, c->kz, *dt, *coefficient, rx, ry, rz);
        ...
        double* error)
{
    ...
    JacobiSolve<DEVICE> solveKernel(c->dims, c->u, c->u0, c->r, c->kx, c->ky, c->kz);
        ...
        double* rro)
{
    ...
    CGInitK<DEVICE> kKernel(c->dims, c->w, c->kx, c->ky, c->kz, rx, ry, rz);
    PROFILED_PARALLEL_FOR(c->fullDomain, kKernel, "CG Init K");

    CGCalcRRO<DEVICE> rroKernel(c->dims, c->mi, c->kx, c->ky, c->kz,
            c->z, c->p, c->r, c->u, c->w, c->preconditioner);
    PROFILED_PARALLEL_REDUCE(c->fullDomain, rroKernel, *rro, "CG Init RRO");
}
        ...
        double* pw)
{
    ...
    CGCalcW<DEVICE> kernel(c->dims, c->w, c->p, c->kx, c->ky, c->kz);
        ...
        const int* maxChebyIters)
{
    ...
    ChebyInit<DEVICE> kernel(c->dims, c->p, c->r, c->u, c->mi, c->u0,
            c->w, c->kx, c->ky, c->kz, *theta, *preconditioner);
        ...
        const int* chebyCalcStep)
{
    ...
    ChebyIterate<DEVICE> kernel(c->dims, c->p, c->r, c->u, c->mi,
            c->u0, c->w, c->kx, c->ky, c->kz, c->alphas, c->betas, 
        ...
        int* currentStep)
{
    ...
    PPCGCalcU<DEVICE> uKernel(c->dims, c->sd, c->r, c->u, c->kx, c->ky, c->kz);
...
void TeaLeafChunk<Device>::Plot3d(View<double*,Device> buffer, std::string name)
{
    ...
    for(int ii = 0; ii < dims.z; ++ii)
    {
        for(int jj = 0; jj < dims.y; ++jj)
        {
            for(int kk = 0; kk < dims.x; ++kk)
            {
                double val = buffer(kk+jj*dims.x+ii*dims.x*dims.y);
