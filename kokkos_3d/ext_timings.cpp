...
/*
 *      Very nasty log file parser to fetch timing results.
 */
extern "C"
void ext_log_timings_(char* outLog, char* teaOutPath, int* numtasks)
{
	...
	while(getline(teaOutFile, line))
	{
		...
		if(probSize[0] > 0 && probSize[1] > 0 && probSize[2] > 0) break;
	}

	oss << " " << probSize[0] << "x" << probSize[1] << "x" << probSize[2];
